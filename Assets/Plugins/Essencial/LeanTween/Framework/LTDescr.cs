...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
        if (delay <= 0f && directionLocal != 0f){
            ...
            if (isTweenFinished){
                ...
                if (!isTweenFinished && onCompleteOnRepeat && hasExtraOnCompletes){
    ...
    static void alphaRecursive(Transform transform, float val, bool useRecursion = true){
        ...
        if (useRecursion && transform.childCount > 0){
            foreach (Transform child in transform) alphaRecursive(child, val);
        }
    }
    ...
    static void colorRecursive(Transform transform, Color toColor, bool useRecursion = true){
        ...
        if (useRecursion && transform.childCount > 0){
            foreach (Transform child in transform) colorRecursive(child, toColor);
        }
    }
...
//		val = (val * val * val * val - 2f);
    ...
    */
    ...
    static void textAlphaRecursive(Transform trans, float val, bool useRecursion = true){
        ...
        if (useRecursion && trans.childCount > 0){
            foreach (Transform child in trans) textAlphaRecursive(child, val);
        }
    }
